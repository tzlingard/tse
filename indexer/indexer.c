...
void sumWords(void* word) {
	...
	if( (doc=(docs_t*)qsearch(((word_t*)word)->freq, pageCompare, (void*)&id)) !=NULL){  
...
//returns the number of times a target word is found in the html of a given page
int wordCount(webpage_t* page, char* target){
  ...
  while ((pos = webpage_getNextWord(page, pos, &word)) > 0) {
    ...
    if (strlen(word) > 0) {
      if (strcmp(word, target)==0){//if the current word matches the target
				count++;
			}
		}
		free(word);
